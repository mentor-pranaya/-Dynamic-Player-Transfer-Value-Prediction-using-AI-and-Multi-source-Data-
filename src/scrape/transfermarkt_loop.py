...
def scrape_competition(base_url, competition_name):
    all_players, all_clubs, all_values, all_competitions, all_ids = [], [], [], [], []
    page = 1
    ...
    while True:
        ...
        for row in rows:
            ...
            # Extract Transfermarkt Player ID
            player_url = player_tag["href"] if player_tag else ""
            # Example href: "/lionel-messi/profil/spieler/28003"
            player_id = None
    ...
    return all_players, all_clubs, all_values, all_competitions, all_ids

# -----------------------------------------
# STEP 4: Scrape All Competitions
# -----------------------------------------
players, clubs, values, competitions_list, ids_list = [], [], [], [], []

for comp_name, comp_url in competitions.items():
    print(comp_url, comp_name)
    p, c, v, comp, ids = scrape_competition(comp_url, comp_name)
    players.extend(p)
    clubs.extend(c)
    values.extend(v)
