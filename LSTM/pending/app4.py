...
if uploaded:
    ...
    if "X" in st.session_state and st.button("Train Model"):
        ...
        st.session_state["y"] = y
        st.session_state["player_index"] = player_index
        st.session_state["target_index"] = 0  # assuming column 0 = market_value
    ...
    if "model" in st.session_state:
        ...
        player_ids = sorted(set(player_index))
        chosen_player = st.selectbox("Select a player for forecast", player_ids)
        ...
        if chosen_player:
            ...
            # prepare last n_steps sequence for forecast
            mask = np.array(player_index)==chosen_player
            X_player = X[mask]  # all sequences of this player
            if len(X_player)==0:
                st.warning("No data for this player.")
            else:
                last_seq = X_player[-1:,:,:]  # most recent window
                n_future = st.slider("How many future windows?",1,6,3)
                preds_future = forecast_future(model, last_seq, scaler, n_future)
                st.write(f"Predicted next {n_future} market values:")
                st.dataframe(pd.DataFrame(preds_future,columns=["forecast_value"]))
                st.line_chart(pd.DataFrame({
                    "past_actual": pd.Series(df_player.actual),
                    "forecast": pd.Series(
                        [None]*(len(df_player.actual)-1)+list(preds_future))
                }))
...
"""
Add to forcast section
target_index = st.session_state["target_index"]
chosen_player = st.selectbox("Select a player for forecast", sorted(set(player_index)))
...
if len(X_player)==0:
    ...
    last_seq = X_player[-1:,:,:]
    n_future = st.slider("Future windows",1,6,3)
    preds_future = forecast_future_multivariate(
        model,
        last_seq,
        scaler,
        n_future=n_future,
    ...
    st.write(f"Predicted next {n_future} market values:")
    st.dataframe(pd.DataFrame(preds_future,columns=["forecast_value"]))
    st.line_chart(pd.DataFrame({"forecast":preds_future}))
