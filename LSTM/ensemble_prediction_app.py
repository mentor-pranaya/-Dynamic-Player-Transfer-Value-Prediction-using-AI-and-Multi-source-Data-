...
if st.sidebar.button("Click here to start training and evaluation"):
    ...
    # Only inverse_transform market_value column
    y_val_true = scaler.inverse_transform(
        np.hstack([y_val[:, [0]], np.zeros((y_val.shape[0], X_val.shape[2]-1))])
    )[:, 0]

    y_val_pred_true = scaler.inverse_transform(
        np.hstack([y_val_pred_lstm[:, [0]], np.zeros((y_val_pred_lstm.shape[0], X_val.shape[2]-1))])
    )[:, 0]

    # Plot Actual vs Predicted
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(y_val_true[:50], label="Actual", color="blue")
    ax.plot(y_val_pred_true[:50], label="Predicted (LSTM)", color="orange", linestyle="--")
    ...
    rmse = np.sqrt(mean_squared_error(y_val_true, y_val_pred_true))
    mae = mean_absolute_error(y_val_true, y_val_pred_true)

    st.metric("RMSE (LSTM)", f"{rmse:.2f}")
    st.metric("MAE (LSTM)", f"{mae:.2f}")
db.close()
