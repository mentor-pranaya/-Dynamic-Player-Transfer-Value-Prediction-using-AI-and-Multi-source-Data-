...
# ---------- Streamlit UI ----------
mode = st.sidebar.radio("Select Input Mode", ["Select Existing Player", "Add New Player"])
forecast_placeholder=st.empty()
forecast_container = st.container()
# Dynamic placeholder
forecast_placeholder = forecast_container.empty()

if st.session_state.trained == False:
    forecast_placeholder.empty()
    with forecast_placeholder.container():
        st.markdown(f"<h4>🔄 Spinning the neurons for first run... please wait</h4>", unsafe_allow_html=True)
        st.write("You may choose to scroll down, if you care to see the action ;)  \n")
        prog_ui=st.progress(0)
        prog_ui.progress(1/5)
        placeholder = st.empty()
        with placeholder:
            st.empty()
            st.image("https://i.gifer.com/Lf7Q.gif", width=400)
else:
    forecast_placeholder.empty()
    with forecast_placeholder.container():
        if mode=="Select Existing Player":
            st.markdown(f"<h4>🔮Loading Player Forecast</h4>", unsafe_allow_html=True)
            #st.write("You may choose to scroll down, if you care to see the action ;)  \n")
            prog_ui=st.progress(0)
            prog_ui.progress(0)
            placeholder = st.empty()
            with placeholder:
                st.empty()
                st.image("https://i.gifer.com/XDZT.gif", width=400)
        else:
            st.markdown(f"<h4>🔮Waiting for data</h4>", unsafe_allow_html=True)
            st.write('Please fill the details and click "🚀 Generate Forecast for New Player" button ')
            prog_ui=st.progress(0)
            prog_ui.progress(0)
            placeholder = st.empty()
            with placeholder:
                st.empty()
                st.image("https://i.gifer.com/4q43.gif", width=400)

st.divider()
st.subheader("Scroll down to see training data and download trained models:")
st.divider()
...
if mode=="Add New Player":
    # --- Custom Player Forecast Section in Sidebar ---
    st.sidebar.markdown("## 🧩 Custom Player Forecast")

    with st.sidebar.expander("➕ Add New Player Details", expanded=True):
        new_player_name = st.text_input("Player Name", value="John Doe")
        n_future = st.slider("Forecast Steps Ahead", 1, 10, 5)
        ...
        for feature in [
            ...
            custom_features[feature] = st.number_input(
                ...
                min_value=0.0, value=float(feature_means.get(feature, 0))
            )

        st.markdown("#### Enter Last 3 Market Values (€ millions)")
        last_3_values = [
            st.number_input(f"Market Value {i+1}", min_value=0.0, value=10.0)
            for i in range(3)
        ]
        # Input features for custom player
        #custom_features = {
        #    "market_value": st.number_input("Market Value (€)", min_value=0.0, step=100000.0, value=5000000.0),
        #    "total_injuries": st.number_input("Total Injuries", min_value=0, step=1,value=0),
        #    "sentiment_mean": st.number_input("Sentiment Mean", min_value=-1.0, max_value=1.0, step=0.01,value=0.5),
        #    "avg_cards_per_match": st.number_input("Average Cards per Match", min_value=0.0, step=0.01),
        #    "avg_days_out": st.number_input("Average Days Out", min_value=0.0, step=0.1),
        #    "recent_injury": st.selectbox("Recent Injury", [0, 1]),
        #    "days_since_last_injury": st.number_input("Days Since Last Injury", min_value=0, step=1),
        #    "minutes_played": st.number_input("Minutes Played", min_value=0, step=1),
        #    "shots_per90": st.number_input("Shots per 90", min_value=0.0, step=0.1),
        #    "pressures_per90": st.number_input("Pressures per 90", min_value=0.0, step=0.1)
        #}

        generate_custom_forecast = st.button("🚀 Generate Forecast for New Player")
...
prog_ui.progress(5/5)
time.sleep(1)
forecast_placeholder.empty()
if mode=="Select Existing Player":
    with forecast_placeholder.container():
        st.markdown(f"<h4>Loading {n_future}-Step Forecast for Player: {player_choice} ...</h4>", unsafe_allow_html=True)
        prog_ui.progress(0)
        with placeholder:
            st.empty()
            st.image("https://i.gifer.com/XDZT.gif", width=400)
    ...
    with forecast_placeholder.container():
        if generate_custom_forecast:
            st.markdown(f"<h4>🔮Loading Custom Forecast</h4>", unsafe_allow_html=True)
            #st.write('Please fill the details and click "🚀 Generate Forecast for New Player" button ')
            st.write("Crunching the numbers, please wait ..")
            prog_ui.progress(0)
            with placeholder:
                st.image("https://i.gifer.com/XDZT.gif", width=400)
        else:
    ...
    if mode=="Select Existing Player":
        ...
        if len(player_data) >= n_steps:
                ...
                if mode=="Select Existing Player":
                    ...
                    def add_labels(x, y, color):
                        for i, val in enumerate(y):
                            val_million = val / 1e6  # convert euros to millions
                            ax.text(
                                x[i], val,
                                f"€{val_million:.2f}M",    # format as €x.xxM
                                color=color, fontsize=9,
                                ha='center', va='bottom',
                                fontweight='bold'
                            )

                    add_labels(steps[:len(actual_last)], actual_last, "blue")
                    add_labels(steps, preds_seq_inv, "orange")
                    add_labels(steps, preds_xgb_inv, "green")
                    add_labels(steps, preds_ensemble_inv, "red")
                    ...
                    forecast_df = pd.DataFrame({
                        ...
                        "Prediction": preds_ensemble_inv
                    })
                    # Ensure numeric columns are floats
                    forecast_df["Prediction"] = pd.to_numeric(forecast_df["Prediction"], errors='coerce')
                        # Display with € Millions formatting
                        #st.write(f"### {n_future}-Step Forecast for Player: {player_choice}")
                    st.dataframe(
                        forecast_df.style.format({
                            "Step Ahead": lambda x: f"Transfer {x}",
                            "Prediction": lambda x: f"€{x/1000000:.2f}M" if pd.notna(x) else ""
                        }), 
                        hide_index=True
                    )                
                    st.pyplot(fig)
            ...
            # Interactive Plot
                    ...
                    actual_last_m = np.array(actual_last) / 1e6
                    ...
                    fig.add_trace(go.Scatter(
                        x=steps[:len(actual_last_m)],
                        y=actual_last_m,
                        mode='lines+markers',
                        name='Actual',
                        line=dict(color='blue', width=2),
                        marker=dict(size=8),
                        hovertemplate='Step %{x}<br>Actual: <br>€%{y:.2f}M<br><extra></extra>'
                    ))
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        marker=dict(size=8),
                        hovertemplate='Seq2Seq LSTM: <br>€%{y:.2f}M<br><extra></extra>'
                    ))
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        marker=dict(size=8),
                        hovertemplate='XGBoost refined: <br>€%{y:.2f}M<br><extra></extra>'
                    ))

                    # Ensemble
                    fig.add_trace(go.Scatter(
                        x=steps,
                        y=preds_ensemble_inv_m,
                        mode='lines+markers',
                        name='Ensemble (avg)',
                        line=dict(color='red', width=2, dash='dot'),
                        marker=dict(size=8),
                        hovertemplate='Ensemble (avg): <br>€%{y:.2f}M<br><extra></extra>'
                    ))

                    # Layout styling
                    fig.update_layout(
                        title=f"{n_future}-Step Forecast for Player {player_choice}",
                        xaxis_title="Steps Ahead",
                        yaxis_title="Transfer Value (€ Millions)",
                        template="plotly_white",
                        legend_title="Models",
                        hovermode='x unified',  # Shows all models’ tooltips together
                        height=600
                    )
                ...
                with tab2:
                    # Keep normalized features for plotting
                    ...
                    # Compute stats on normalized data
                    max_norm = normalized_df.max()
                    mean_norm = normalized_df.mean()
                    player_norm = normalized_df.loc[df["player_id"] == pid].iloc[-1]

                    # Keep original (human-readable) values separately
                    player_orig = df[df["player_id"] == pid].iloc[-1][feature_cols]
                    max_orig = df[feature_cols].max()
                    mean_orig = df[feature_cols].mean()

                    comparison_df_orig = pd.DataFrame({
                        "Feature": feature_cols,
                        "Max": max_orig.values,
                        "Mean": mean_orig.values,
                        f"{player_choice}": player_orig.values
                    })

                    st.write(f"### 🔍 Feature Comparison (Original Values) for Player: {player_choice}")
                    # Convert 'market_value' column in Euro Millions
                    # Function to format market_value row
                    def format_row(row):
                        new_row = row.copy()
                        if row["Feature"] == "market_value":
                            new_row["Max"] = f"€{row['Max']/1e6:.2f}M"
                            new_row["Mean"] = f"€{row['Mean']/1e6:.2f}M"
                            new_row[f"{player_choice}"] = f"€{row[player_choice]/1e6:.2f}M"
                        else:
                            # Format numeric columns to 2 decimals
                            for col in ["Max", "Mean", f"{player_choice}"]:
                                new_row[col] = f"{row[col]:.2f}"
                        return new_row

                    comparison_df_formatted = comparison_df_orig.apply(format_row, axis=1)
                ...
                with tab3:
                    ...
                    fig.add_trace(go.Bar(
                        x=comparison_df_orig["Feature"], 
                        y=max_norm.values,
                        name="Max", marker_color="lightgray",
                        text=[f"{v:.2f}" for v in max_orig.values],  # original values as text
                    ...
                    fig.add_trace(go.Bar(
                        ...
                        text=[f"{v:.2f}" for v in mean_orig.values],
                        textposition='outside'
                    ))
                    fig.add_trace(go.Bar(
                        x=comparison_df_orig["Feature"], 
                        y=player_norm.values,
                        name=f"{player_choice}", marker_color="orange",
                        text=[f"{v:.2f}" for v in player_orig.values],
                        textposition='outside'
                    ))

                    fig.update_layout(
                        title=f"Feature-wise Comparison for {player_choice}",
                        barmode='group',
                        xaxis_tickangle=-45,
                ...
                with tab4:
                    cursor=db.cursor()
                    cursor.execute("""
                        select Season, transfer_date as `Transfer Date`, club_left as `Club Left`, club_joined as `Club Joined`, mv_raw as `Market Value`, fee_raw as Fee  from player_transfer_history where transfermarkt_id=%s
                    """,(pid,))
                    df_trfr = pd.DataFrame(cursor.fetchall(), columns=[desc[0] for desc in cursor.description])
                    cursor.close()
                    st.dataframe(df_trfr,hide_index=True)
                    st.write(f"Data Source: [Transfermarkt](https://www.transfermarkt.com/spieler/transfers/spieler/{pid})")
                    #st.page_link(f"https://www.transfermarkt.com/spieler/transfers/spieler/{pid}","Source")
        else:
